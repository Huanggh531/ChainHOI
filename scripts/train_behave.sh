#train on behave on gpu[2]
python src/train.py trainer.devices=\'2,\'  data=behave_gcn data.joint_format=behave_joint_5 logger=tensorboard \
    callbacks.model_checkpoint.save_top_k=5 callbacks.model_checkpoint.save_last=False data.batch_size=16 trainer.max_epochs=400 \
    callbacks.model_checkpoint.monitor=Metrics/contact_distance_mean callbacks.model_checkpoint.mode=min model=chainhoi_behave \
