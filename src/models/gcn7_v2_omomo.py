...
from .metrics import TM2TMetrics, MMMetrics,ContactMetric2,PeneMetric
...
class GCN(L.LightningModule):
    def __init__(self,
        ...
        self.sample_scheduler.set_timesteps(200)
    ...
    def configure_evaluator_and_metrics(self, dataset_name, evaluator):
        ...
        # self.t2m_mm_metric = MMMetrics(mm_num_times=30, dist_sync_on_step=True)
        from src.models.evaluator.MotionCLIP import MotionCLIPEvaluator2
        self.t2m_evaluator=MotionCLIPEvaluator2(dataset_name,evaluator.T2M_dir)
        self.t2m_metrics = TM2TMetrics(diversity_times=200 if dataset_name == "hml3d" else 100,
                                dist_sync_on_step=True)
        #self.t2m_mm_metric = MMMetrics(mm_num_times=30, dist_sync_on_step=True)
        #self.human_metric=metric_compute=ComputeMetrics(njoints=22,jointstype="humanml3d")
        #self.obj_metric=ComputeMetrics_obj(njoints=22,jointstype="humanml3d")
        self.concat_metric=ContactMetric2(dist_sync_on_step=True)
    ...
    def calculate_dis_loss(self, output, length, all_obj_points, motion_unorm_gt, faces, contact, contact_pro,padding_mask, weight=1):
        ...
        for b in range(obj_point.shape[0]):
            ...
            # 计算 L1 Loss 
            mse_loss = F.mse_loss(torch.zeros_like(distances[0]), distances[0], reduction='none')
            # 创建掩码矩阵
    ...
    def _step_network(self, batch, batch_idx):
        ...
        gcn_motion, length, text,all_obj_points,contact_pro = batch["gcn_motion"], batch["motion_len"], batch["text"],batch["obj_points"],batch["contact_pro"]
        faces=batch["faces"]
        ...
        # if self.obj_flag:
        #     with torch.no_grad():
        #         obj_emb = self.objEmbedding(obj_points.permute(0, 2, 1)) # [bs, n_points, d]

        if self.obj_flag:
            obj_emb = self.objEmbedding(all_obj_points) # [bs, n_points, d]
        with torch.no_grad():
        ...
        dis_loss = self.calculate_dis_loss(output, length, all_obj_points, motion_unorm_gt, faces,
                                           contact, contact_pro,padding_mask, weight=2.0)
        # dis_loss,obj_loss = self.calculate_dis_loss(output, length, all_obj_points, motion_unorm_gt, faces,
        #                                    contact, contact_pro,padding_mask, weight=1.0)
        ...
        return {"loss": diffusion_loss+dis_loss,"difuss_loss":diffusion_loss,"dis_loss":dis_loss}
        #return {"loss": difuss_loss+obj_loss,"difuss_loss":difuss_loss,"obj_loss":obj_loss}
        #return {"loss": difuss_loss}
    ...
    def training_step(self, batch, batch_idx):
        ...
        #all_loss=losses["loss"]+losses["sdf_loss"]+losses["dis_loss"]
        return losses["loss"]
    ...
    def on_validation_epoch_end(self):
        ...
        #metric_output2=self.human_metric.compute(sanity_flag=self.trainer.sanity_checking)
        #metric_output3=self.obj_metric.compute(sanity_flag=self.trainer.sanity_checking)
        ...
        #results.update({f"Metrics/{key}": value.item() for key, value in metric_output2.items()})
        #results.update({f"Metrics/{key}": value.item() for key, value in metric_output3.items()})
        results.update({f"Metrics/{key}": value.item() for key, value in metric_output4.items()})
        self.t2m_metrics.reset()
        #self.human_metric.reset()
        #self.obj_metric.reset()
    ...
    def evaluate(self, batch, split='val'):
        ...
        if self.trainer.sanity_checking:
            return {}
        gcn_motion, length, text,contact,all_obj_points,faces = batch["gcn_motion"], batch["motion_len"], batch["text"], batch["contact"],batch["obj_points"],batch["faces"]
        ...
        if self.obj_flag:
            with torch.no_grad():
                # obj_emb = self.objEmbedding(obj_points.permute(0,2,1))
                obj_emb = self.objEmbedding(all_obj_points)
        ...
        pred_gcn_motion = self.sample_motion(gcn_motion, length, text,obj_emb,all_obj_points,faces,contact,dataset,k,batch["seq_name"])
        gt_motion = dataset.to_hml3d_format(gcn_motion).float()
        pred_motion = dataset.to_hml3d_format(pred_gcn_motion)
        sixD=pred_motion[...,269:273] 
        ...
        #self.human_metric.update(pred_motion_human,gt_motion_human,length)
        #self.obj_metric.update(pred_obj,gt_obj,obj_points,obj_name,length)
        ...
        T=rot.shape[1]
        # 使用 PyTorch 的矩阵乘法
        #all_obj_points=batch["obj_points"]
        obj_point = torch.matmul(all_obj_points.unsqueeze(1).expand(-1, T, -1, -1), rot.transpose(2, 3)) + trans.unsqueeze(2)
        #self.concat_metric.update(contact_pro,length,human_positions,obj_point,faces,contact,obj_name,dataset,batch["idx"])
        self.pene_metric.update(None,contact_pro,length,human_positions,obj_point,faces)
       
    
    
    def sample_motion(self, gt_gcn_motion, length, text,obj_emb,all_obj_points,faces,contact,dataset,k,obj_name):
        ...
        for i, t in enumerate(time_steps):
            ...
            # mu_t,var_t = self.obtain_mu_t(pred_noise,t,pred_gcn_motion)
            # pred_gcn_motion = mu_t +  (var_t ** 0.5) * pred_gcn_motion
            ...
            for b in range(pred_gcn_motion.shape[0]):
                afford = np.load(os.path.join("/home/guohong/HOI-Diff/omomo/guidance3",obj_name[b]+".npy"))
