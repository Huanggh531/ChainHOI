...
def euler_to_rotation_matrix(angles):
    ...
    batch_size, T = angles.shape
    Ry = torch.zeros((batch_size, T, 3, 3), dtype=torch.float, device=angles.device)
...
def recover_from_6d(data,sixD ,joints_num):
    
    r_matrix = euler_to_rotation_matrix(sixD[...,0])
    ...
    '''Add root XZ to joints'''
    positions[..., 0] += sixD[..., 1:2]
    positions[..., 2] += sixD[..., 3:4]

    '''Concate root and joints'''
    positions = torch.cat([sixD[...,1:4].unsqueeze(-2), positions], dim=-2)
    return positions
...
def rotation_vector_to_matrix(rot_vec):
    """
    将旋转向量转换为旋转矩阵。
    :param rot_vec: 旋转向量 tensor [T, 3]
    :return: 旋转矩阵 tensor [T, 3, 3]
    """
    T = rot_vec.shape[0]
    angle = torch.norm(rot_vec, dim=-1, keepdim=True)
    axis = rot_vec / (angle + 1e-8)  # 防止除零
    cos_angle = torch.cos(angle).unsqueeze(-1)
    sin_angle = torch.sin(angle).unsqueeze(-1)
    one_minus_cos = (1.0 - cos_angle).unsqueeze(-1)

    x, y, z = axis[:, 0], axis[:, 1], axis[:, 2]
    xx, yy, zz = x * x, y * y, z * z
    xy, yz, zx = x * y, y * z, z * x
    xs, ys, zs = x * sin_angle.squeeze(), y * sin_angle.squeeze(), z * sin_angle.squeeze()

    rot_matrix = torch.zeros((T, 3, 3), device=rot_vec.device, dtype=rot_vec.dtype)
    rot_matrix[:, 0, 0] = cos_angle.squeeze() + xx * one_minus_cos.squeeze()
    rot_matrix[:, 0, 1] = xy * one_minus_cos.squeeze() - zs
    rot_matrix[:, 0, 2] = zx * one_minus_cos.squeeze() + ys
    rot_matrix[:, 1, 0] = xy * one_minus_cos.squeeze() + zs
    rot_matrix[:, 1, 1] = cos_angle.squeeze() + yy * one_minus_cos.squeeze()
    rot_matrix[:, 1, 2] = yz * one_minus_cos.squeeze() - xs
    rot_matrix[:, 2, 0] = zx * one_minus_cos.squeeze() - ys
    rot_matrix[:, 2, 1] = yz * one_minus_cos.squeeze() + xs
    rot_matrix[:, 2, 2] = cos_angle.squeeze() + zz * one_minus_cos.squeeze()
...
def rotation_vector_to_matrix(rot_vec):
    """
    将旋转向量转换为旋转矩阵。
    :param rot_vec: 旋转向量 tensor [batch_size, T, 3]
    :return: 旋转矩阵 tensor [batch_size, T, 3, 3]
    """
    batch_size, T, _ = rot_vec.shape
    angle = torch.norm(rot_vec, dim=-1, keepdim=True)
    axis = rot_vec / (angle + 1e-8)  # 防止除零
    ...
    # 初始化旋转矩阵
    rot_matrix = torch.zeros((batch_size, T, 3, 3), device=rot_vec.device, dtype=rot_vec.dtype)

    # 填充旋转矩阵
    rot_matrix[..., 0, 0] = (cos_angle + xx * one_minus_cos).squeeze(-1)
    rot_matrix[..., 0, 1] = (xy * one_minus_cos - zs).squeeze(-1)
    rot_matrix[..., 0, 2] = (zx * one_minus_cos + ys).squeeze(-1)
    rot_matrix[..., 1, 0] = (xy * one_minus_cos + zs).squeeze(-1)
    rot_matrix[..., 1, 1] = (cos_angle + yy * one_minus_cos).squeeze(-1)
    rot_matrix[..., 1, 2] = (yz * one_minus_cos - xs).squeeze(-1)
    rot_matrix[..., 2, 0] = (zx * one_minus_cos - ys).squeeze(-1)
    rot_matrix[..., 2, 1] = (yz * one_minus_cos + xs).squeeze(-1)
    rot_matrix[..., 2, 2] = (cos_angle + zz * one_minus_cos).squeeze(-1)
...
# def recover_root_rot_pos2(data):
#     rot_vel = data[..., 0]
#     r_rot_ang = torch.zeros_like(rot_vel).to(data.device)
#     '''Get Y-axis rotation from rotation velocity'''
#     r_rot_ang[..., 1:] = rot_vel[..., :-1]
...
#     r_rot_quat, r_pos = recover_root_rot_pos2(data)
#     positions = data[..., 4:(joints_num - 1) * 3 + 4]
#     positions = positions.view(positions.shape[:-1] + (-1, 3))
